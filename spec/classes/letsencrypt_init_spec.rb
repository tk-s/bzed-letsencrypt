require 'spec_helper'

# rubocop:disable LineLength
describe 'letsencrypt', type: :class do
  let(:pre_condition) do
    file { '/opt/letsencrypt/requests/spectest/': ensure => directory }
  end

  let :facts do
    {
      osfamily: 'Debian',
      fqdn: 'spectest',
      puppetmaster: 'spec.test',
      letsencrypt_crts: 'spec.test',
      letsencrypt_csr_spectest: '-----BEGIN CERTIFICATE REQUEST-----\\n-----END CERTIFICATE REQUEST-----',
    }
  end

  it { is_expected.to contain_class('::letsencrypt::certificate') }
end
# rubocop:enable LineLength
